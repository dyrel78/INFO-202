...
public class ProductViewerTest {
    ...
    public void setUp() {
        ...
        when(dao.searchById("TestID1")).thenReturn(p1);
        when(dao.searchById("TestID2")).thenReturn(p2);
        when(dao.searchById("TestID3")).thenReturn(p3);
...
//    }
    ...
    public void testSearchById() {
        ...
        var model = (SimpleListModel) fixture.list("lstProducts").target().getModel();
    ...
    public void filterByCategory() {
        ...
        // select p1's category 
        fixture.comboBox().selectItem(p1.getCategory());

        // verify that filterByCategory was called on the DAO, and that p2's category was the parameter that was passed
        verify(dao).filterByCategory(p1.getCategory());

        // verify that the JList is now displaying p1
        fixture.list("lstProducts").requireItemCount(1);
        SimpleListModel model = (SimpleListModel) fixture.list().target().getModel();
        assertThat(model, containsInAnyOrder(p1));
    }
